...
class WithdrawEthLog extends Component {
  ...
  async onHeaderRefresh() {
    ...
    let res = await Axios.get(
      "/wallet/withdraws?page=0&size=10&startDate=" +
    ...
    return (
      <View style={styles.loglist}>
        <View style={styles.lineCom}>
          <Text style={styles.txtName}>提现时间：</Text>
          <Text style={styles.txtInfo}>{data.item.createTime}</Text>
        </View>
        <View style={styles.lineCom}>
          <Text style={styles.txtName}>提现数量：</Text>
          <Text style={[styles.txtInfo, styles.txtNum]}>
            {new BigNumber(data.item.outQuantity).toFormat()}
            {data.item.itemType}
          </Text>
        </View>
        <View style={styles.lineCom}>
          <Text style={styles.txtName}>提现地址：</Text>
          <Text style={styles.txtInfo}>{data.item.address}</Text>
        </View>
        <View style={styles.lineState}>
          {data.item.status == "INIT" ? (
            <Text style={styles.ing}>审核中</Text>
          ) : data.item.status == "SUCCESS" || data.item.status == "DONE" ? (
            <Text style={styles.success}>审核通过</Text>
          ) : (
            <Text style={styles.fail}>提现被拒绝</Text>
          )}
        </View>
      </View>
    );
        ...
        <View style={styles.top}>
          ...
          >
            <View style={styles.nav}>
              ...
              <Text style={styles.titleM}>提现明细ETH</Text>
